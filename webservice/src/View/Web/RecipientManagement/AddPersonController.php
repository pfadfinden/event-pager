...
#[Route('/recipients/person/add', name: 'web_recipient_management_person_add')]
...
final class AddPersonController extends AbstractController
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        $form = $this->createFormBuilder($recipientRequest)
            ...
            ->add('save', SubmitType::class, ['label' => 'Add Person'])
            ->getForm();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $id = Ulid::generate();
            $this->commandBus->do(new CreateRecipient($id, 'person', $recipientRequest->name));

            $this->addFlash('success', t('Person created successfully'));

            return $this->redirectToRoute('web_recipient_management_person_details', ['id' => $id]);
        }

        return $this->render('recipient-management/person/add.html.twig', [
            'form' => $form,
        ]);
    }
}
