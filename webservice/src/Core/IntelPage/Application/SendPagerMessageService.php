...
final readonly class SendPagerMessageService implements SendPagerMessageServiceInterface
{
    ...
    public function nextMessageToSend(): ?PagerMessage
    {
        $dql = sprintf(
            'SELECT m FROM %s m WHERE m.attemptedToSend < %d AND m.transmittedOn is null AND m.queuedOn > :maxAge ORDER BY m.priority ASC, m.queuedOn ASC',
            PagerMessage::class,
            self::RETRY_LIMIT
        );
